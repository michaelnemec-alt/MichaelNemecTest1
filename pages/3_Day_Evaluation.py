...
st.title("🎯 Day Evaluation — Shift Decision Support")
st.markdown(
...
with st.sidebar:
    ...
    if sf_available:
        data_source = st.radio("Picking data source", ["CSV Upload", "Snowflake"], index=0)
    else:
...
# ── Scissors Chart ───────────────────────────────────────────────────────────
st.header("📐 Scissors — Plan vs Actual (Cumulative)")
...
st.download_button(
    "⬇️ Download Scissors Chart",
    data=fig_to_bytes(fig_sc),
    file_name=f"scissors_{pick_date}.png",
    mime="image/png",
)
plt.close(fig_sc)
...
st.download_button(
    "⬇️ Download Buffer Chart",
    data=fig_to_bytes(fig_buf),
    file_name=f"buffer_{pick_date}.png",
    mime="image/png",
)
plt.close(fig_buf)
...
# ── Next-Day Pre-Pick Analysis ───────────────────────────────────────────────
st.header("🌙 Next-Day Pre-Picking")
...
# ── Per AutoStore breakdown ──────────────────────────────────────────────────
st.divider()
st.header("🏭 Per AutoStore Breakdown")
