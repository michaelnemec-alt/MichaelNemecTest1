...
with st.sidebar:
    ...
    selected_view = st.selectbox("Dashboard view", VIEWS, index=0)
...
def format_pivot_index(pivot, mode):
    if mode == "Week":
        pivot.index = pivot.index.strftime("W%V %Y")
    elif mode == "Month":
        pivot.index = pivot.index.strftime("%Y-%m")
...
if selected_view == "Overview & Health":
    with st.spinner("Loading system health..."):
        df_health = load_for_sites(query_system_health)
    ...
    st.download_button("⬇️ Download health data", data=csv_bytes, file_name=f"health_{dt_from}_{dt_to}.csv", mime="text/csv")


# ══════════════════════════════════════════════════════════════════════════════
# VIEW: ERROR & HEALTH METRICS
# ══════════════════════════════════════════════════════════════════════════════
elif selected_view == "Error & Health Metrics":
    with st.spinner("Loading uptime, robot state, and health data..."):
        df_health = load_for_sites(query_system_health)
        df_uptime = load_for_sites(query_uptime)
        df_robot = load_for_sites(query_robot_state)
        df_port_uptime = load_for_sites(query_port_uptime)
        df_incidents = load_for_sites(query_incidents)
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    with st.spinner("Loading performance data..."):
        df_health = load_for_sites(query_system_health)
        df_pwt = load_for_sites(query_port_wait_time_daily)
    ...
    if not df_pwt.empty:
        ...
        if df_filtered.empty:
            ...
            for metric, label, ylabel, thresh, thresh_label in [
                ...
                piv = format_pivot_index(piv, aggregation)
            ...
            piv_count = format_pivot_index(piv_count, aggregation)
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    if metric_choice in df_health.columns:
        ...
        pivot = aggregate_pivot(df_health, metric_choice, aggregation)
        st.plotly_chart(make_trend_chart(pivot, metric_choice.replace("_", " ").title(), metric_choice, pct=pct), use_container_width=True)
