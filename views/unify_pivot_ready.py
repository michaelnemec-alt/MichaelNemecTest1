...
def _make_output_name(input_name):
...
def _process_file(raw_bytes, filename):
    df = pd.read_csv(io.BytesIO(raw_bytes))
    return _process_dataframe(df)


def _show_result(result, output_name, key_suffix):
    ...
    st.download_button(f"Download {output_name}", data=csv_bytes,
                       file_name=output_name, mime="text/csv", key=f"download_{key_suffix}")
...
def render():
    ...
    with st.sidebar:
        ...
        api_date_from = api_date_to = None
        ...
        if data_source == "CubeAnalytics API":
            ...
            if installations:
                ...
                selected_idx = st.selectbox("Installation", range(len(install_labels)),
            ...
            uploaded_files = st.file_uploader("Upload CSV files", type=["csv"],
                                              ...
                                              help="Unify Analytics / CubeAnalytics CSV exports.",
                                              key="unify_csvs")
            st.divider()
            st.markdown(
                "**Required columns:**\n"
    ...
    if data_source == "CubeAnalytics API":
        if not api_installation:
            st.info("Select an installation in the sidebar.")
            return
        ...
        with st.spinner(f"Loading data for {api_installation['name']}..."):
            try:
                df_raw = query_port_wait_time(api_installation["id"], str(api_date_from), str(api_date_to))
            except Exception as e:
        ...
        result, error = _process_dataframe(df_raw)
        if error:
        ...
        st.divider()
        _show_result(result, output_name, "api_0")
    else:
        if not uploaded_files:
            st.info("Upload CSV file(s) in the sidebar to get started.")
            st.markdown(
                """
                ---
                ### How it works
                ...
                - Group by: Date x Hour x Pick type x Port ID x Category
                - **Weighted averages** for bin wait time and operator handling time
                - Estimate **Average open ports** = (bin_wait + op_handling) x Count / 3600 (capped at 1)
        ...
        for i, uploaded_file in enumerate(uploaded_files):
            ...
            output_name = _make_output_name(uploaded_file.name)

            with st.expander(f"{uploaded_file.name} -> {output_name}", expanded=(i == 0)):
                result, error = _process_file(raw, uploaded_file.name)
                if error:
                    st.error(f"Error: {error}")
                    continue
                _show_result(result, output_name, f"csv_{i}")
